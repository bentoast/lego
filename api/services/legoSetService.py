...
class LegoSetService:
    ...
    def getSetById(self, setid) -> LegoSet:
        dbResults = self.databaseService.executeQuery(
            'SELECT * FROM LegoSet WHERE SetId = %s', (setid,))
        if dbResults is None or len(dbResults) == 0:
            return None
        return LegoSet(dbResults[0])

    def hasSet(self, setid) -> bool:
        dbResults = self.databaseService.executeQuery(
           'SELECT COUNT(*) AS SetCount FROM LegoSet WHERE SetId = %s',
           (setid,))
        if dbResults is None or len(dbResults) == 0:
            return False
    ...
    def disableCheck(self, setid):
        return self.databaseService.executeNonQuery(
            'UPDATE LegoSet SET CanCheck = \'f\' WHERE SetId = %s', (setid,))
    ...
    def getFilteredSets(self, filter, order, asc, count, page):
        ...
        # This is math. We want to see if the SetId starts
        # with the same digits as what we're passing
        # If we pass 41, we want 41234, 4140, 418954, etc.
        # We can express all numbers as n * 10^x.
        # 41 is 4.1 * 10^1, 41234 is 4.1234 * 10^4, etc.
        # The difference of exponents tells us the magnitude
        # we need to change the number by
        # We can find the exponents by taking the log(base 10) of the numbers.
        # 4 - 1 is 3. 10^3 is 1000. So we divide the SetId by 1000 and
        # floor it to get 41, then we can compare
        if 'setid' in filter:
        ...
        countResult = self.databaseService.executeQuery(
            countStatement, params)[0]
        ...
        return (
            countResult['count'],
            [LegoSet(current) for current in results])
