...
class ScrapingService:
    ...
    def ExtractSetInfo(self, element):
        ...
        hrefString = element.xpath(
            './/a[@data-test="product-leaf-image-link"]/@href')
        if hrefString is not None and len(hrefString) > 0:
            # The URL will look like
            # en-us/product/blah-blah-12345 or en-us/product/12345
        ...
        if currentSet.setid is not None:
            name = element.xpath(
                './/h1[@data-test="product-overview-name"]/span/text()')
            name.extend(element.xpath(
                './/a[@data-test="product-leaf-title"]/span/text()'))
            for currentName in name:
                currentSet.name = currentName.encode(
                    'ascii', errors='ignore').decode()

            flags = element.xpath('.//span[@data-test="product-flag"]/text()')
            flags.extend(element.xpath(
                './/span[@data-test="product-leaf-badge"]/text()'))
            ...
            price.extend(element.xpath(
                './/span[@data-test="product-leaf-price"]/text()'))
            ...
            priceOld = element.xpath(
                './/span[@data-test="product-price-sale"]/text()')
            priceOld.extend(element.xpath(
                './/span[@data-test="product-leaf-discounted-price"]/text()'))
            for currentOld in priceOld:
                modifiedOld = Decimal(currentOld[1:])
                currentSet.price = modifiedOld

            discount = element.xpath(
                './/div[@data-test="sale-percentage"]/text()')
            discount.extend(element.xpath(
                './/span[@data-test="product-leaf-discount-badge"]/text()'))
