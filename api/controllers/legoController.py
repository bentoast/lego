...
class LegoController:
    ...
    def MultipleRequest(self, request):
        ...
        (allCount, allSets) = self.legoSetService.getFilteredSets(
            filter, request['order'], 'asc' in request and request['asc'],
            count,
            page)
        print(f'{{ "total": {allCount}, "page": {page},'
              f'"results": [{",".join([ls.toJson() for ls in allSets])}]}}')
